...
func (s *Server) handleUsage(ctx context.Context, req *mcp.CallToolRequest, in usageInput) (*mcp.CallToolResult, any, error) {
	...
	b.Bullet("`status()` — branch, changes, open PRs, board, attention-needed alerts")
	...
	b.Bullet("`setup(cleanup?)` — configure new repo: branches, protection, convention labels. Returns label inventory + agent instructions to walk user through removing GitHub defaults, adding project-specific labels, and creating loom.yml. cleanup=true auto-removes GitHub defaults")
