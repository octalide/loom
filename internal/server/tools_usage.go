...
func (s *Server) handleUsage(ctx context.Context, req *mcp.CallToolRequest, in usageInput) (*mcp.CallToolResult, any, error) {
	...
	b.Bullet("`finish(wait?)` — ready PR + merge/auto-merge + cleanup. wait=true blocks until merged (waits for CI). Only use wait when subsequent work depends on the merge result")
	b.Bullet("`wait(pr, timeout?)` — wait for a PR to merge. Polls every 3s, default 90s timeout. Blocks for CI duration — skip if work can be done in parallel")
	b.Text("")
